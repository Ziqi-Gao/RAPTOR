...
class DataProcessing():
    ...
    def dispacher(self):
        ...
        elif self.data_name == 'hatexplain':
            p, q = self.hatexplain()
            prompt = 'According to the comment, tell whether they present hate speech or not.'
        else:
    ...
    def get_prompt(self, prompt, cot, question):
        ...
        elif self.data_name in ['STSA','coinflip','IMDb','hateeval','hatexplain']:
    ...
    def hatexplain(self):
        # NOTE: Binary mapping: hatespeech + offensive as positive, normal as negative.
        pos_labels = {0, 2}  # 0=hatespeech, 1=normal, 2=offensive
        label_map = {"hatespeech": 0, "normal": 1, "offensive": 2}

        dataset_path = "./data/hatexplain"
        ...
        for row in ds:
            ...
            if counts.count(max_count) > 1:
                # Skip ties (e.g., 1-1-1).
                continue
            major = counts.index(max_count)

            if major in pos_labels:
