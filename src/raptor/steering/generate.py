...
Sequential Steering Algorithm:
    ...
    5:             epsilon <- (sigmoid^(-1)(P_target) - b - w^T*e)   # Adaptive magnitude (negative to steer to negative)
    6:             e <- e + epsilon*v                            # Apply steering perturbation
    7:     end if
    8: end for
...
def compute_adaptive_epsilon(
    ...
    Formula: epsilon = I(should_steer) * (sigmoid^(-1)(P_target) - b - w^T*e)
    ...
    # For target_prob=0.0001 (negative): logit_target approx -9.21
    # If current_logit is large positive (P_m approx 1.0), epsilon will be large negative
            ...
            for l = 1 to L do:
                if TestAcc(P_m) > P_1 then:
                    ...
                    if should_steer(e, P_target) then:
                        epsilon <- (sigmoid^(-1)(P_target) - b - w^T*e)  # Adaptive magnitude
                        e <- e + epsilon*v                        # Apply perturbation
    ...
    for layer in sorted_layers:
        ...
        if test_accuracies is not None and test_accuracies[layer] is not None:
            if test_accuracies[layer] <= accuracy_threshold:
                ...
                print(f"  [Layer {layer}] TestAcc={test_accuracies[layer]:.4f} > P_1 [ok]")
...
def load_hatexplain_samples(
    ...
    *,
    ...
    # Resolve a writable datasets cache location (priority: env -> HF_HOME -> local fallback).
    cache_dir = (
        os.environ.get("HF_DATASETS_CACHE")
        or os.environ.get("HF_DATASETS_CACHE_DIR")
    )
    if not cache_dir:
        hf_home = os.environ.get("HF_HOME")
        if hf_home:
            cache_dir = str(Path(hf_home) / "datasets")
        else:
            cache_dir = str(Path.cwd() / ".hf_cache" / "datasets")
    ...
    if datapath and datapath.exists() and datapath.is_dir():
        ...
        print(f"Loading hatexplain dataset from Hugging Face (cache: {cache_dir})")
...
def print_dataset_config_summary(dataset_tag: str, selected_manners: Optional[Sequence[str]] = None) -> None:
    ...
    for manner, config in steering_configs.items():
        ...
        is_selected = " [ok]" if selected_manners and manner in selected_manners else ""
...
def build_fixed_total_prompt_specs(
    total_trials: int,
    manners: Sequence[str],
    dataset_tag: str,
    *,
    fixed_text: str = "Write a brief neutral statement.",
) -> list[PromptSpec]:
    """Build prompt specs without dataset source texts (fixed total trials).

    This mode is designed for experiments where generations should not depend
    on dataset examples. `source` is a synthetic trial id.
    """
    if total_trials <= 0:
        raise ValueError(f"total_trials must be > 0, got {total_trials}")
    ...
    for manner in manners:
        ...
        if instruction_template is None:
            raise ValueError(
                f"No instruction template found for manner '{manner}' in dataset '{dataset_tag}'"
            )

        if "{text}" in instruction_template:
            instruction = instruction_template.format(text=fixed_text)
        elif "{manner}" in instruction_template:
            instruction = instruction_template.format(manner=manner)
        else:
            instruction = instruction_template
        ...
        for trial_idx in range(total_trials):
            specs.append(
                PromptSpec(
                    label=manner,
                    prompt=prompt,
                    source=f"trial_{trial_idx:04d}",
                    use_chat_template=None,
                    meta={
                        "manner": manner,
                        "dataset": dataset_tag,
                        "sampling_mode": "fixed_total",
                        "trial_id": trial_idx,
                    },
                )
            )
...
def build_stsa_manual_prompt_specs(
    prompt_json_path: Path,
    manners: Sequence[str],
) -> list[PromptSpec]:
    """Build STSA prompt specs from a user-provided JSON prompt list.

    Expected JSON format:
    {
      "final_prompts": [
        {"prompt_text": "...", "focus": "...", "style": "..."},
        ...
      ]
    }
    """
    ...
    if not prompt_json_path.is_file():
        raise FileNotFoundError(f"STSA prompt json not found: {prompt_json_path}")

    with open(prompt_json_path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    items = payload.get("final_prompts", [])
    if not isinstance(items, list) or not items:
        raise ValueError(f"Invalid STSA prompt json: missing non-empty 'final_prompts' list in {prompt_json_path}")
    ...
    if not cleaned:
        raise ValueError(f"No valid prompt_text found in {prompt_json_path}")

    specs: list[PromptSpec] = []
    for item in cleaned:
        instruction = str(item["prompt_text"])
        prompt = PROMPT_TEMPLATE.format(instruction=instruction)
        for manner in manners:
            specs.append(
                PromptSpec(
                    label=manner,
                    prompt=prompt,
                    source=instruction,  # keep source_text field behavior consistent
                    use_chat_template=None,
                    meta={
                        "manner": manner,
                        "dataset": "STSA",
                        "sampling_mode": "stsa_manual_prompts",
                        "focus": item["focus"],
                        "style": item["style"],
    ...
    print(f"[STSA prompts] Loaded {len(cleaned)} manual prompts from {prompt_json_path}")
    return specs
...
def generate_unified_steering_csv(
    *,
    ...
    print(f"[INFO] Processing {len(unique_prompts)} unique prompts x {config.num_samples} samples x {len(all_manners)} manners")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--savepath", type=Path, default=Path("./exp_results"))
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional STSA manual prompt JSON path. If set with dataset=STSA and sampling-mode=source, prompts come from this file.",
    )
    parser.add_argument(
        "--sampling-mode",
        choices=["source", "fixed_total"],
        default="source",
        help="source: existing source_text-driven flow; fixed_total: ignore dataset source text and run fixed total trials.",
    )
    parser.add_argument(
        "--total-trials",
        type=int,
        default=64,
        help="Total trial prompts per manner for --sampling-mode fixed_total.",
    )
    parser.add_argument(
        "--fixed-text",
        type=str,
        default="Write a brief neutral statement.",
        help="Text used only when instruction templates contain {text} in fixed_total mode.",
    )
...
def main() -> None:
    ...
    # sampling_mode:
    #   - source: existing dataset-source-driven flow
    #   - fixed_total: no dataset source text, fixed total trial prompts
    if args.sampling_mode == "fixed_total":
        if args.total_trials <= 0:
            raise ValueError(f"--total-trials must be > 0 for fixed_total mode (got {args.total_trials})")
        if args.num_samples != 1:
            print(
                f"[Sampling] fixed_total mode ignores --num-samples={args.num_samples}; "
                f"using 1 so total rows stay fixed."
            )
        prompt_manners = [manners[0]] if use_unified else list(manners)
        prompts = build_fixed_total_prompt_specs(
            total_trials=args.total_trials,
            manners=prompt_manners,
            dataset_tag=args.dataset_tag,
            fixed_text=args.fixed_text,
        )
        effective_num_samples = 1
        print(
            f"[Sampling] Mode=fixed_total, trials_per_manner={args.total_trials}, "
            f"prompt_manners={tuple(prompt_manners)}"
        )
    else:
        if args.dataset_tag.upper() == "STSA" and args.stsa_prompt_json is not None:
            prompt_manners = [manners[0]] if use_unified else list(manners)
            prompts = build_stsa_manual_prompt_specs(args.stsa_prompt_json, prompt_manners)
            effective_num_samples = args.num_samples
            print(f"[STSA prompts] Using --num-samples={effective_num_samples} per manual prompt.")
        else:
