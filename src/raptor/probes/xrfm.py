...
Adapter around the xRFM probe implementation.
...
try:
    ...
    raise ImportError(
        "xRFM baseline requires `pip install git+https://github.com/dmbeaglehole/xRFM.git@773fae8`."
    ) from e
...
def _compute_prediction_metrics(pred_proba, y_true) -> Dict[str, float]:
    y_np = _as_numpy(y_true).reshape(-1, 1)
    p_np = _as_numpy(pred_proba).reshape(y_np.shape)
    pred = np.where(p_np >= 0.5, 1, 0)
    labels = np.where(y_np >= 0.5, 1, 0)
    acc = float(np.sum(pred == labels) / len(pred) * 100.0)
    true_pos = float(np.sum((pred == 1) & (labels == 1)))
    pred_pos = float(np.sum(pred == 1))
    actual_pos = float(np.sum(labels == 1))
    ...
    try:
        metrics["auc"] = float(roc_auc_score(y_np, p_np))
    except ValueError:
            ...
            for center_grads in search_space["center_grads"]:
                try:
                    ...
                    if tuning_metric == "top_agop_vectors_ols_auc":
                        ...
                        metrics = _compute_prediction_metrics(pred_proba, val_y)
...
def run_xrfm(
    ...
    *,
    ...
    if best_model is None and device_str == "cuda":
        ...
        if err_summary:
            print(
                "[xrfm][warn] GPU search failed; retrying on CPU. Example error: "
                f"{err_summary[0].splitlines()[-1]}",
                file=sys.stderr,
            )
        else:
            print("[xrfm][warn] GPU search failed; retrying on CPU.", file=sys.stderr)
        best_model, best_val_score, best_h, concept_vec = _search_on_device("cpu")

    if best_model is None:
        err_summary = best_h.get("errors", []) if isinstance(best_h, dict) else []
        msg = "RFM search failed; no usable model was found."
        if err_summary:
            msg += f" Example error: {err_summary[0].splitlines()[-1]}"
    ...
    test_metrics = _compute_prediction_metrics(test_preds, y_te_t.to(best_model.device))
    test_metric = test_metrics.get(metric_key, test_metrics.get("acc"))

    if test_metric is None:
        raise RuntimeError("Failed to compute the requested test metric.")
